...
import { entrenadorCompleteSchema, type EntrenadorComplete } from "@/lib/validations/entrenador"
import { createEntrenadorProfileAction } from "./actions"

/**
 * Multi-step Entrenador Profile Form
 * Task 3.6.1, REQ-2.2.x
 *
 * 7 Steps (similar to atleta but with professional fields):
 * 1. Datos Personales
 * 2. Datos de Contacto
...
interface EntrenadorFormProps {
  ...
  existingData?: Partial<EntrenadorComplete> | null
}

const STEPS = [
  { id: 1, title: "Datos Personales", description: "Información básica" },
  { id: 2, title: "Contacto", description: "Información de contacto" },
  { id: 3, title: "Datos Profesionales", description: "Experiencia y certificaciones" },
...
export function EntrenadorForm({ userId, asociaciones, existingData }: EntrenadorFormProps) {
  const [currentStep, setCurrentStep] = useState(1)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const router = useRouter()
  const { toast } = useToast()

  const form = useForm<EntrenadorComplete>({
    resolver: zodResolver(entrenadorCompleteSchema),
    defaultValues: {
      user_id: userId,
      estado: "pendiente",
      ...existingData,
  ...
  const onSubmit = async (data: EntrenadorComplete) => {
    setIsSubmitting(true)

    try {
      const result = await createEntrenadorProfileAction(data)
          ...
          {currentStep === 3 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Experiencia, certificaciones y especialidad
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            {...field}
                            placeholder="ej: Velocidad, Fondo, Lanzamientos"
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                ...
                <FormField
                  control={form.control}
                  name="titulos_deportivos"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Títulos Deportivos</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          value={field.value || ""}
                          placeholder="Títulos académicos o deportivos relevantes"
                          rows={3}
                        />
                      </FormControl>
                      <FormMessage />
          ...
          {currentStep === 6 && (
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <FormField
                  control={form.control}
                  name="titulos_profesionales_url"
                  render={({ field }) => (
                    <FormItem>
                      <DocumentUpload
                        label="Títulos Profesionales (Opcional)"
                        description="Títulos académicos o profesionales"
                        accept="image/*,.pdf"
                        currentUrl={field.value || undefined}
                        onUploadComplete={field.onChange}
                        onRemove={() => field.onChange(null)}
                ...
                <FormField
                  control={form.control}
                  name="certificaciones_deportivas_url"
                  render={({ field }) => (
                    <FormItem>
                      <DocumentUpload
                        label="Certificaciones Deportivas (Opcional)"
                        description="Certificados de cursos o capacitaciones"
                        accept="image/*,.pdf"
                        currentUrl={field.value || undefined}
                        onUploadComplete={field.onChange}
                        onRemove={() => field.onChange(null)}
...
function getFieldsForStep(step: number): string[] {
  switch (step) {
    ...
    case 3:
      return ["asociacion_id", "especialidad", "anios_experiencia"]
    case 4:
      return ["tipo_sangre"]
    case 5:
