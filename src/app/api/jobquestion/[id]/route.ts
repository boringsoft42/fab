...
export async function PUT(
  ...
  try {
    ...
    console.log('🔍 API: Backend data received for job question update:', resolvedParams.id);
    return NextResponse.json(data, { status: response.status });
  } catch (error) {
    console.error('Error in job question update route:', error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const data = await response.json();
    console.log('🔍 API: Backend data received for job question deletion:', resolvedParams.id);
    return NextResponse.json(data, { status: response.status });
  } catch (error) {
    console.error('Error in job question deletion route:', error);
    return NextResponse.json(
      { message: 'Internal server error' },
      { status: 500 }
    );
