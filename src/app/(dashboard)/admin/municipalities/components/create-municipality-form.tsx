...
export function CreateMunicipalityForm({ onSuccess }: CreateMunicipalityFormProps) {
  ...
  const handleGenerateCredentials = () => {
    if (watchedName && watchedName.length > 2) {
      ...
      toast({
        title: "Error",
        description: "Debes ingresar el nombre de la institución primero (mínimo 3 caracteres).",
        variant: "destructive",
      });
    }
  };
  ...
  const onSubmit = async (data: CreateMunicipalityFormData) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "No se pudo crear la institución. Verifica que el usuario y email no existan.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {errors.institutionType && (
              <p className="text-sm text-red-600">{errors.institutionType.message}</p>
            )}
          </div>
